...
PACKAGE_FILES := manifest.json background.js options.html options.js options.css rules.js rules.json \
	icons/icon-16.png icons/icon-32.png icons/icon-48.png icons/icon-128.png
PACKAGE_EPOCH ?= $(shell git log -1 --format=%ct 2>/dev/null || printf 315532800)
FIREFOX_WEB_EXT_VERSION := 9.4.0

.PHONY: help test check stage chrome-stage firefox-stage package firefox-lint

help:
	@printf '%s\n' 'Available targets:' '  make test         Run rule-generation tests.' '  make check        Validate JSON and JavaScript syntax.' '  make package      Create the deterministic Chrome Web Store ZIP.' '  make firefox-lint Lint the staged Firefox extension with web-ext.'
...
package: check chrome-stage
	rm -f dist/cf-access-header-injector.zip
	cd dist/chrome-package && TZ=UTC zip -X -q ../cf-access-header-injector.zip $(PACKAGE_FILES)
